...
DEFAULT_TARGETS = {
    ...
    "opencode": {
        "dir": Path(os.environ.get("SKILLS_GOVERNOR_OPENCODE_DIR", HOME / ".config" / "opencode" / "skills")).expanduser(),
        "commands": ["opencode"],
        "markers": [HOME / ".config" / "opencode", HOME / ".opencode"],
    },
    "qoderwork": {
        "dir": Path(
...
HERMES_DEFAULT_CATEGORIES = {
    "gg-ai-native-startup-playbook": "devops",
    "gg-child-psychology-for-content": "creative",
    "gg-fact-driven-ai-methodology": "software-development",
    "gg-harness": "software-development",
    "gg-skills-governor": "devops",
}
...
def iter_target_entries(target_name: str, target_root: Path) -> list[Path]:
    ...
    for entry in sorted(target_root.iterdir()):
        if entry.is_symlink() or entry.is_file():
            ...
            entries.extend(sorted(entry.iterdir()))
    return entries
...
def inspect_target(name: str, target: Path, source: Path, selected_skills: set[str] | None = None) -> dict:
    ...
    if target.exists():
        for entry in iter_target_entries(name, target):
            if not entry.is_symlink():
                continue
            points_to = os.readlink(entry)
            resolved = entry.resolve(strict=False)
            if entry.name in managed_names:
                if selected_skills is not None and entry.name not in selected_skills:
                    continue
                expected = skill_target_path(name, target, entry.name)
                if entry == expected:
                    continue
                if resolved == (source / entry.name).resolve(strict=False) or is_under(resolved, source_resolved):
                    rows.append(
                        {
                            "skill": entry.name,
                            "source": None,
                            "target": str(entry),
                            "status": "stale-managed-link" if entry.exists() else "broken-stale-managed-link",
                            "points_to": points_to,
