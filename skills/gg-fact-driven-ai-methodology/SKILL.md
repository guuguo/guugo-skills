---
name: gg-fact-driven-ai-methodology
...
**Create each folder only when it gets its first real file. Do not pre-scaffold the full layout above as empty directories** — empty folders are clutter and signal structure that does not exist yet. The list is a menu, not a mandatory skeleton.

### When the project already uses gg-harness (co-location)

This skill is a layer, not a parallel filesystem. If the project already runs `gg-harness` (a `docs/harness/` tree, an AGENTS.md map, `docs/harness/rules/`), do **not** spin up a competing 10-folder `.fact-driven-ai/<goal>/` tree beside it. Nest the fact layer into the harness structure and reuse what harness already owns:

- **Location**: keep the fact layer under the harness home (e.g. `docs/harness/facts/`), or, if a slim `.fact-driven-ai/<account-or-goal>/` is already established, keep it but trim it. Either way the working set is usually just `facts/ evidence/ judgments/ goals/`; add `experiments/` or `reports/` only once they hold real files.
- **Reuse harness, don't duplicate it**: promote L3 validated rules into harness `docs/harness/rules/` (the project's stable-rule home) instead of a separate `methodologies/` folder. Let `fact-packs/` collapse into `reports/`, `runs/` into harness `acceptance.md`/`result.md`, and `archive/` defer to the project's existing archive location. One spec/rule home, not two.
- **Products never live in the fact workspace**: manuscripts, scripts, generated media, shipped docs, datasets — these are deliverables, not facts. They belong in the project's content/asset area; the fact layer only references them by path. The fact workspace holds facts/evidence/judgments **about** the work, never the work itself. (Common category error: a novel/script draft sitting inside `reports/`.)
- **Discipline is unchanged**: L0–L3 levels, confidence/status, append-only corrections, and supersession all stay exactly as specified below. Only the folder sprawl collapses and the source-of-truth ownership is shared with harness; the epistemics do not change.
...
```text
gg-fact-driven-ai-methodology: establishes source-of-truth and rebuild logic
technical planning: designs one solution from confirmed facts
execution harness: executes one task and records run evidence
```

Do **not** merge this skill into `gg-harness`. They are deliberately separate layers: harness manages *work* (requirements, plan/acceptance, evidence contract); this skill manages *truth* (facts vs judgments, confidence, drift control). Collapsing them buries the anti-drift discipline inside a requirements flow.

When both are active in one project, ownership splits cleanly:

- **This skill owns the epistemics**: what is L0/L1/L2/L3, confidence, append-only corrections, supersession.
- **Harness owns the filesystem and stable rules**: `docs/harness/` is the single home; validated L3 rules live in `docs/harness/rules/`; requirements live in `docs/harness/specs/`. The fact layer nests inside (see "When the project already uses gg-harness"), it does not start a parallel tree.
- **The project's content/asset area owns deliverables**: products are referenced by the fact layer, never stored inside it.
...
- Pre-scaffolding the full folder layout as empty directories instead of creating each folder on first real use.
- Starting a parallel `.fact-driven-ai/` tree next to an existing `gg-harness` `docs/harness/` structure, or duplicating its rules/specs home, instead of nesting into it.
- Storing deliverables (manuscripts, scripts, generated media, shipped docs) inside the fact workspace instead of the project's content/asset area.
